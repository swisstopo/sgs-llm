...
const MODEL_OPTIONS: readonly ModelPreference[] = ['primary', 'secondary'];
...
export class SgsComposer extends LitElement {
    ...
    return html`
      ...
      <div class="input-row">
        ...
        ${this.busy
          ...
          : html`<button class="submit" ?disabled=${this.disabled} @click=${this.send}>
              ${t('chat.send')}
            </button>`}
      </div>
    `;
  ...
  private renderModelOption(model: ModelPreference) {
    ...
    return html`
      ...
      >
        ...
        <span class="selected">${selected ? checkIcon : nothing}</span>
      </button>
    `;
  }
  ...
  private renderModelLogo(model: ModelPreference) {
    ...
    return html`<span class="model-logo" aria-hidden="true">
      <svg
        viewBox="0 0 24 24"
        fill=${model === 'primary' ? '#D97757' : '#FA520F'}
  ...
  private modelName(model: ModelPreference): string {
    return t(model === 'primary' ? 'chat.model.primary' : 'chat.model.secondary');
  }
